...
CREATE TABLE PACJENT (PESEL INTEGER NOT NULL PRIMARY KEY, IMIE TEXT, NAZWISKO TEXT, PLEC TEXT, WIEK INTEGER, WIEK_CAT TEXT);
...
SELECT P.PLEC, P.WIEK_CAT, M.GRUPA, M.WARTOSC, strftime("%d", M.DATA) AS DZIEN, strftime("%m", M.DATA) AS MIESIAC, strftime("%Y", M.DATA) AS ROK FROM POMIAR AS M LEFT JOIN PACJENT AS P ON M.PESEL == P.PESEL;